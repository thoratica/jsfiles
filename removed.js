; (async () => {
  ...
  const postId = (await (await (await fetch(`/api/discuss/find?username=${user.username}&title=&sort=created&rows=0&page=1&noCache=${Date.now()}&removed=true`)).json()).data).map(post => post._id)
  const postList = []
